...
from resnet18_7_plot.dataset import MyDataset_EM, MyDataset_EM_nobox
from resnet18_7_plot.model import ResNet101
...
def BulidDataloader(args, flag='train'):
    ...
    # Set Data Path
    dataPath_prefix = '/home/lab501/face_database/RAFDB/'    #your-data-path
    ...
    if args.Dataset == 'RAF':
        ...
        if args.Distribute == 'Basic':
            if args.Aligned:
                ImagePath = dataPath_prefix + 'aligned/'
            ...
            Train_List = dataPath_prefix + 'list_patition_label.txt'
            Test_List = dataPath_prefix + 'list_patition_label.txt'
            #BBoxsPath = dataPath_prefix + 'RAF/basic/Annotation/boundingbox/'
        ...
        Train_List = '/home/carina/python-lab/datasets/AffectNet-auto/train/'

        Test_List = '/home/carina/python-lab/datasets/AffectNet-auto/val/'
    ...
    if args.Dataset=='RAF':
        ...
        if flag == 'train':
            ...
            for i in range(list_patition_label.shape[0]):
                if list_patition_label[i, 0][:5] == "train":
                    ...
                    #landmark[:, 1] += bbox[1]
                    if args.Aligned:
                        data_imgs.append(ImagePath+list_patition_label[i, 0].split('.')[0]+'_aligned.jpg')# s.split('.')[0] + '_aligned.jpg'
                    else:
            ...
            for i in range(list_patition_label.shape[0]):
                if list_patition_label[i, 0][:4] == "test":
                    ...
                    # landmark[:, 1] += bbox[1]
                    if args.Aligned:
                        data_imgs.append(ImagePath + list_patition_label[i, 0].split('.')[0]+'_aligned.jpg')
                    else:
                        data_imgs.append(ImagePath + list_patition_label[i, 0])
        ...
        if args.Aligned:                     
            dataset = MyDataset_EM_nobox(data_imgs, data_labels, data_landmarks, flag, needAU, args.Model, trans, target_trans)
        else:
        ...
        # Dataset with Bbox
        dataset = MyDataset_EM(data_imgs, data_labels, data_landmarks, flag, args.Model, trans, target_trans)
        ...
        # Dataset
        needAU = False if args.Experiment == 'EM' else True                    
        dataset = MyDataset_EM(data_imgs, data_labels, data_landmarks, flag, needAU, args.Model, args.Experiment, trans, target_trans)
        ...
        # Dataset
        needAU = False if args.Experiment == 'EM' else True                    
        dataset = MyDataset_EM(data_imgs, data_labels, data_landmarks, data_bboxs, flag, needAU, args.Model, args.Experiment, trans, target_trans)
        ...
        da = torchvision.datasets.ImageFolder(root=List_Path)
        labels = da.targets
        ...
        for i in range(labels.shape[0]):
            if da_img[i][1] == 0:
                ...
                if expression_0 > max_samples:
                    ...
                    data_imgs.append(da_img[i][0])
                    data_labels.append(da_img[i][1])
        ...
        # Dataset Distribute
        if flag == 'train':
            print('The train dataset distribute: %d, %d' % (len(data_imgs), len(data_labels)))
        elif flag == 'test':
            print('The test dataset distribute: %d, %d' % (len(data_imgs), len(data_labels)))

        # Dataset with Bbox
        dataset = MyDataset_EM_nobox(data_imgs, data_labels, data_landmarks, flag, needAU, args.Model, trans,
                                     target_trans)
        # dataset = MyDataset_EM_AFF(data_imgs, data_labels, flag)
        # DataLoader
        if flag == 'train':
            data_loader = data.DataLoader(dataset=dataset, batch_size=args.Train_Batch_Size, shuffle=True,
                                          num_workers=args.Num_Workers, drop_last=True)
            # data_loader = data.DataLoader(dataset=dataset, batch_size=args.Train_Batch_Size, shuffle=False, sampler=ImbalancedDatasetSampler(dataset),   num_workers=args.Num_Workers, drop_last=True)
        elif flag == 'test':
            data_loader = data.DataLoader(dataset=dataset, batch_size=args.Test_Batch_Size, shuffle=True,
...
def Set_Param_Optim(args, model):
    ...
    if args.Experiment == 'EM': 
        ...
        #    param.requires_grad = True
        #for param in model.reduce_dim_em2.parameters():
        #    param.requires_grad = True
        #for param in model.reduce_dim_em3.parameters():
        #    param.requires_grad = True
        #for param in model.pred_em.parameters():
        #    param.requires_grad = True 
        #for param in model.gcn_1.parameters():
        #   param.requires_grad = True
        #for param in model.gcn_2.parameters():
        #    param.requires_grad = True
    ...
    # Feature Fuse Experiment
        ...
        for param in model.pred_em_fuse_3.parameters():
            param.requires_grad = True
        for param in model.pred_em_fuse_4.parameters():
            param.requires_grad = True
        # for param in model.pred_em.parameters():
        #    param.requires_grad = True
